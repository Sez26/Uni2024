...
def S_curve_a(reference,max_a): #  this can't be done on seperate thetas, because then the thetas would be shifted from eachother
...
def S_curve_v(reference,max_v,max_a): #  this can't be done on seperate thetas, because then the thetas would be shifted from eachother
    ref = np.copy(reference)
    # find shape of ref (assume first column is for time series)
    (num_int,cols) = np.shape(ref)
    # number of theta signals (col-1)
